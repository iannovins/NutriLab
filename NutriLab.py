...
import datetime
import calendar
from zoneinfo import ZoneInfo # Aggiunto per gestire il fuso orario italiano
...
# DB di emergenza (aggiunto l'ottavo valore: peso medio per pezzo)
FALLBACK_DB = {
...
def load_database():
    try:
        ...
        if 'Var_Cottura' not in df.columns:
            df['Var_Cottura'] = 0.0
        # Aggiunta colonna per il peso medio dei pezzi se non esiste
...
# ==========================================
    ...
    with c1:
        ...
        # ORA E IMPOSTAZIONI ORARIE CON FUSO ORARIO ITALIANO
        # Utile per non sfasare l'orario se l'app è ospitata su server all'estero
        try:
            tz_italy = ZoneInfo("Europe/Rome")
            ora_attuale = datetime.datetime.now(tz_italy).time()
        except:
...
# ==========================================
    ...
    db_var = c8.number_input("% V.Cott", value=st.session_state.get("db_var_cottura", 0.0), step=1.0)
    db_peso_pz = c9.number_input("Peso 1pz", value=st.session_state.get("db_peso_pz", 50.0), step=1.0, help="Gradi medi per 1 pezzo/unità")
